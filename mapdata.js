class MapFile {
  constructor(filename, splitname, chapter, coop, triggers, wikicontent) {
  ...
  selfStr(readable, includeWiki) {
    ...
    if (readable) {
      ...
      return `new MapFile("${this.filename}", "${this.splitname}", ${this.chapter}, ${this.coop}, [${triggers}]${includeWiki ? `,\n\`${this.wikicontent}\``: ""})`;
    } else {
      triggers = triggers.join(",");
      return `new MapFile("${this.filename}","${this.splitname}",${this.chapter},${this.coop},[${triggers}]${includeWiki ? `,\`${this.wikicontent}\`` : ""})`;
    }
  }
  
  createMtriggerString() {
    return `
sar_speedrun_cc_start ${this.splitname.replaceAll(" ", "")} map=${this.filename} action=split
...
async function updateWikiData() {
	let titles = [];
	maps.forEach(e => titles.push(e.wikiname));
	//maximum titles count is 50, split them into chunks
	for (let i = 0; i < Math.ceil(titles.length / 50); i++) {
		let j = i * 50;
		let titleChunk = titles.slice(j, j + 50).join("|");
		//api query from fucking hell. took me 2 hours to find the origin option
		let url = `https://wiki.portal2.sr/api.php?action=query&format=json&origin=*&prop=revisions&titles=${titleChunk}&rvprop=content&rvslots=main`;
		let time = new Date();
		let response = await queryAPI(url);
		console.log(`got wikitext for maps ${j}-${Math.min(maps.length - 1, j + 50)} (${formatBytes(response.length)}), took ${new Date() - time}ms`)
...
function addMaps() {
	...
	for (let map of maps) {
		// if the start trigger is missing, insert it
		if (map.triggers[0] != `"Start" load action=force_start`) map.triggers.splice(0, 0, `"Start" load action=force_start`);
		// same for the various ending triggers
		if (map.coop) {
			if (map.triggers[map.triggers.length - 1] != `"Flags 2" flags "ccafter=Flags 1" action=stop`) {
				map.triggers.push(`"Flags 1" flags`);
				map.triggers.push(`"Flags 2" flags "ccafter=Flags 1" action=stop`);
			}
		} else if (map.triggers[map.triggers.length - 1] != `"Flags" flags action=stop`) {
			map.triggers.push(`"Flags" flags action=stop`);
		}
	}
}