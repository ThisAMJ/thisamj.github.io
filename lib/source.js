...
const src = {
	...
	key: {
		...
		keyPress: function(ev, down) {
			ev.preventDefault();
			if (ev.repeat) return;

			if (!this.list.html.includes(ev.code)) {
				if (down) this.parent.con.warn(`You just pressed ${ev.code}, which isn't registered. Tell AMJ!\n`);
				return;
			}

			let code = this.list.html.indexOf(ev.code);
		...
		exec: function(args, onlyIfExists) {
			...
			// for (let line of lines) {
				...
				// while (this.parent.cmd.buffer.length > j)
					// this.parent.cmd.execute(this.cmd.buffer.splice(j, 1)[0]);
			// }
	...
	con: {
		...
		flush: function() {
			if (!!this.output) {
				...
				while (this.buffer.length > 0) {
					...
					if (this.parent.cmd.cvar('developer') >= text[1]) {
		...
		print: function(text = '', debugLevel = 0, color = '#FFF') {
			if (this.parent.cmd.cvar('developer') >= debugLevel) {
		...
		autocomplete: function(cmd) {
			...
			if (args.length === 1 && !cmd.endsWith(' ')) {
				let convars = this.parent.cmd.convars.filter(e => !this.parent.cmd.isFlagSet(e, FCVAR_DEVELOPMENTONLY) && !this.parent.cmd.isFlagSet(e, FCVAR_HIDDEN));
	...
	cmd: {
		...
		unregister: function(name) {
			...
			this.convars = this.convars.filter(e => e.name.toLowerCase() !== name.toLowerCase());
		},

		getConvar: function(name) {
			// let convar = this.convars[name.toLowerCase()];
			let convar = this.convars.find(e => e.name.toLowerCase() === name.toLowerCase());
		...
		createCommand: function(name, helpStr = '\n', flags = FCVAR_NONE, callback = undefined, autocompletefunc = cmd => [], synchronous = true) {
			...
			let existing = this.convars.findIndex(e => e.name.toLowerCase() === name.toLowerCase());
			if (~existing) {
				if (this.convars[existing].isCommand) return this.convars[existing] = obj;
				return this.parent.con.err(`Tried to create command over cvar\n`);
			}
			this.convars.push(obj);
		},
		...
		printConvarDescription: function(convar) {
			...
			if (convar.isCommand) {
				...
				let min = convar.min, max = convar.max;
				out += `= "${convar.value}"`;
		...
		describeCvar: function(convar) {
			return this.printConvarDescription(convar);
			// if (typeof convar === 'string') return this.describeCvar(this.getConvar(convar));
			// if (!convar || convar.isCommand) return '';
			// let str = `"${convar.name}" = "${convar.value}" `;
			// if (convar.value !== convar.default) str += `(def. "${convar.default}") `;
			// if (!isNaN(convar.min)) str += `min. ${convar.min} `;
			// if (!isNaN(convar.max)) str += `max. ${convar.max} `;
			// if (convar.helpStr !== '') str += `- ${convar.helpStr}`;
			// return str;
		},
		...
		awaitReadyExecute: function(cmd, flush = false) {
			if (src.cmd.commandReady) {
				src.cmd.execute(cmd);
				if (src.cmd.buffer.length > 0 && !src.cmd.slowdown) src.cmd.awaitReadyExecute(src.cmd.buffer.shift(), true);
				else if (src.cmd.slowdown) src.cmd.executing = false;
			} else {
				setTimeout(src.cmd.awaitReadyExecute, 1, cmd, !src.cmd.slowdown);
			}
		},
		
		awaitExecuteComplete: function() {
			if (src.cmd.buffer.length === 0) {
				src.cmd.executing = false;
			} else {
				setTimeout(src.cmd.awaitExecuteComplete, 1);
			}
		},
		
		isFlagSet: function(cmd, flag) {
	...
	__init: function() {
		...
		this.cvar = function(...args) {this.cmd.cvar(...args)};
...
const CON_COMMAND = function(...args) {
	args.splice(2, 0, FCVAR_NONE);
	src.cmd.createCommand(...args);
};
const CON_COMMAND_F = function(...args) {src.cmd.createCommand(...args)};
const CON_CVAR = function(...args) {src.cmd.createCvar(...args)};
const CON_BUTTON = function(name, helpStrA = '', helpStrB = '', flags = FCVAR_NONE) {
	src.cmd.createCommand(`+${name}`, helpStrA, flags, args => {});
	src.cmd.createCommand(`-${name}`, helpStrB || helpStrA, flags, args => {});
}
