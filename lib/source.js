...
const source = {
	...
	/** Utility functions for console commands */
	__: {
	...
	keyUp: function(event) {
		...
		let bind = this.binds.find(e => e.key == this.__.keys[this.__.htmlKeys.indexOf(event.code)].toUpperCase());
		if (!bind) return;
		this.__.pressedKeys = this.__.pressedKeys.filter(e => e != bind.key);
	...
	cmd: {
		 ...
		 * @param {String} text 
		 */
		parseArguments: function(text = '', breakset = "(){}:'") {
			let out = [];
				...
				*/
				...
				while (i < cmd.length) {
					if (cmd[i] == '"') while (++i < cmd.length && cmd[i] != '"') arg += cmd[i];
						...
						arg += cmd[i];
						while (++i < cmd.length && !(~breakset.indexOf(cmd[i]) || cmd[i] == ' ')) arg += cmd[i];
					}
					if (~breakset.indexOf(cmd[i])) {
						args.push(arg);
						arg = cmd[i];
		...
		execute: function(args) {
			...
			profiling.push(this.buffer.length);
	...
	cfg: {
		// TODO:
		// This is kinda webpage-specific, see if you can't generalise
		currentView: '',
	...
	createCommand: function(name, helpStr = '\n', callback = (args) => {}) { //source.con.warn(`NotImpl: '${args.cmdStr.trimStart()}'\n`)) {
...
3: mouse_acceleration = pow(raw_mouse_delta, m_customaccel_exponent - 1) * sensitivity`);
		...
		cvr('cl_showbattery', 0, 'Draw current battery level at top of screen when on batter power\n');
		...
		cmd('BindToggle', 'Performs a bind <key> "increment var <cvar> 0 1 1"\n');
		cmd('stuffcmds', 'Parses and stuffs command line + commands to command buffer.\n');
		cmd('echo', 'Echo text to console.\n');
		cmd('alias', 'Alias a command.\n');
		cmd('whitelistcmd', 'Runs a whitelisted command.\n');
		cmd('cmd', 'Forward command to server.\n');
		cmd('exec', 'Execute script file.\n');
		cmd('execifexists', 'Execute script file if file exists.\n');
		...
		cvr('developer', 0);
		cvr('name', 'Chell', 'Current user name\n'); // normally defaults to 0, but that's boring
		...
		cmd('exec', 'exec <filename> [path id] : execute a script file\n', function(args) {
			...
			s = args[1];
			source.con.log(`Execing config: ${s}\n`, 1);
			if (!s.endsWith('.cfg')) s += '.cfg';
			txt = source.cfg.get(args[1]);
			
			if (txt == null) {
				// Fail silently for autoexec, joystick and game
				if (!~['autoexec.cfg', 'joystick.cfg', 'game.cfg'].indexOf(s))
					source.con.err(`exec: Couldn't exec ${s}. Are you sure it exists?\n`);
				return;
			}
			
			if (txt.length > 1048576) // 1024 ** 2
				return source.con.err(`exec: ${s} larger than 1 MB!\n`);
			...
			// for (let line of lines) {
				// let j = source.cmd.buffer.length;
				// source.cmd.executeCommand(line, false, true);
				// while (source.cmd.buffer.length > j)
					// source.cmd.execute(source.cmd.buffer.splice(j, 1)[0]);
			// }
			
			// Alternatively, we could just add the entire text at once
			source.cmd.executeCommand(txt);
		});
	...
	__init: function() {
		...
		this.__initVars();
		this.con.debugLevel = this.cvar('developer');
		
		this.onTickEvents.pre.push(() => this.con.flush());
		requestAnimationFrame(this.__tick);
		
		window.addEventListener('load', function() {
			source.cfg.add('autoexec', true, `// AMJ's web implementation of the Source engine for P2.
...
setTimeout(() => {
	source.cmd.executeCommand('exec autoexec');
}, 500);
