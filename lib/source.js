	...
	cfg: {
		...
		writeConfiguration: function(iController, filename) {
			if (this.parent.cmd.convars.hasOwnProperty('joystick_force_disabled') &&
				...
				if (this.parent.cmd.convar('joystick_force_disabled') != this.parent.cmd.convar('joystick_force_disabled_set_from_options')) {
					this.parent.cmd.convar('joystick_force_disabled') = this.parent.cmd.convar('joystick_force_disabled_set_from_options');
				}
			}
			...
			for (let key of this.parent.key.list.src) {
				...
				configBuff += `bind "${key}" "${pszBinding.command}"\n`;
			}

			for (let cvar of this.parent.cmd.cvars) {
