...
const src = ({
	...
	key: {
		...
		bind: function(args) {
			...
			if (this.list.src.indexOf(b) < 1) {
				// TODO: add joystick keys to lists
				if (!b.startsWith('JOY') && !b.startsWith('POV_') && !~b.indexOf(' AXIS ')) {
		...
		keyPress: function(event, down) {
			...
			// I've added it for all because it's useful
	...
	cfg: {
		 ...
		 */
		get: function(name) {
			return this.cfgs[name.replaceAll('.cfg', '').toLowerCase()] ? this.cfgs[name.replaceAll('.cfg', '').toLowerCase()] : null;
		},
		...
		set: function(name, value = '') {
			this.cfgs[name.replaceAll('.cfg', '').toLowerCase()] = value;
		},
		...
		remove: function(name) {
			delete this.cfgs[name.replaceAll('.cfg', '').toLowerCase()];
		},
		...
		wasConfigCfgExecuted: false,

		readConfiguration: function(iController, readDefault) {
			let filename = 'config.cfg'
			let saveconfig = false;
			if (this.get(filename) === null) {
				filename = 'config_default.cfg';
				saveconfig = true;
			}
			src.cmd.executeCommand(`exec ${filename}`);

			if (this.parent.key.binds.length == 0) {
				// UseDefaultBindings();
			}

			this.parent.key.bindKey('ESCAPE', 'cancelselect');

			if (!this.parent.key.binds.find(e => e.cmd === 'toggleconsole')) {
				this.parent.key.bindKey('`', 'toggleconsole');
			}

			// SetupDefaultAskConnectAcceptKey();
			this.wasConfigCfgExecuted = true;

			if (saveconfig) {
				this.writeConfiguration(iController, 'config.cfg');
			}
		},

		writeConfiguration: function(iController, filename) {
			if (this.parent.cmd.convars.hasOwnProperty('joystick_force_disabled') &&
				this.parent.cmd.convars.hasOwnProperty('joystick_force_disabled_set_from_options')) {
				if (this.parent.cmd.cvar('joystick_force_disabled') != this.parent.cmd.cvar('joystick_force_disabled_set_from_options')) {
					this.parent.cmd.cvar('joystick_force_disabled') = this.parent.cmd.cvar('joystick_force_disabled_set_from_options');
				}
			}

			if (!filename) filename = "config.cfg";
			if (!this.wasConfigCfgExecuted) return;
			if (false /* g_bInEditMode */) {
				return this.parent.con.log(`skipping ${filename} output when in map edit mode\n`);
			}
			if (this.parent.key.binds.length <= 1) {
				return this.parent.con.log(`skipping ${filename} output, no keys bound\n`);
			}
			let configBuff = 'unbindall\n';
			for (let key of this.parent.key.list.src) {
				let pszBinding = this.parent.key.binds.find(e => e.key === key);
				if (!pszBinding || !pszBinding.cmd) continue;
				// TODO: different slot support?
				configBuff += `bind "${key}" "${pszBinding.cmd}"\n`;
			}

			for (let cvarname in this.parent.cmd.convars) {
				let cvar = this.parent.cmd.convars[cvarname];
				if (cvar.isCommand) continue;
				if (this.parent.cmd.isFlagSet(cvar, FCVAR_ARCHIVE)) {
					configBuff += `${cvar.name} "${cvar.value}"\n`;
				}
			}

			for (let hh = 0; hh < 2 /* host_state.max_splitscreen_players */; ++hh) {
				configBuff += `cmd${hh + 1} +jlook\n`;
			}

			this.set(filename, configBuff);
			this.parent.con.log(`Host_WriteConfiguration: Wrote ${filename}\n`);
		}
	...
	con: {
		...
		autocomplete: function(cmd) {
			...
			if (args.length === 1 && !cmd.endsWith(' ')) {
				...
				if (!src.cmd.showhidden) convars = convars.filter(e => !this.parent.cmd.isFlagSet(e, FCVAR_DEVELOPMENTONLY) && !this.parent.cmd.isFlagSet(e, FCVAR_HIDDEN));
			...
			return cmd.isCommand
				? ((cmd.autocompletefunc || (args => []))(args) || []).map(e => args.cmdStr.slice(0, args.argLengthS[args.length - 2]) + e)
	...
	cmd: {
		...
		printConvarDescription: function(convar) {
			...
			if (convar.helpStr) {
				this.parent.con.log(`${out} - ${convar.helpStr}\n`);
		...
		executeCommand: function(command = '', immediately = true, append = false) {
			this.buffer[append ? 'push' : 'unshift'](...this.parseArguments(command, true));
		...
		execute: function(command) {
			...
			if (cmd) {
				if (cmd.isCommand) {
					...
					if (command.argc === 1) {
						this.parent.con.log(this.printConvarDescription(cmd));
						return true;
					}
					let value = command[1];
	...
	start: function() {
		if (src.con.output) src.con.output.removeAllChildNodes();
		if (!src.cfg.get('config_default.cfg')) {
			src.cfg.set('config_default.cfg', `unbindall
		...
		src.cfg.readConfiguration(-1, false);
