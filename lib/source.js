...
const source = {
	...
	hookPluginLoad: function(name, callback) {
		this.plugin_load_hooks.push({name: name, callback: callback})
	},
	...
	// engine/cmd.cpp L500
	exec: function(args, onlyIfExists, useWhitelist = false) {
		// TODO: Whitelist. Will anyone... ever... use it though?
		if (args.length < 2) {
		...
		if (txt == null) {
			...
			if (!onlyIfExists && !~['autoexec.cfg', 'joystick.cfg', 'game.cfg'].indexOf(k))
				this.con.err(`exec: Couldn't exec ${k}.\n`);
			return;
		}
	...
	cmd: {
		// public/tier1/convar.h L294/295
		maxArgCount: 64,
		maxCmdLength: 512,
		// tier1/convar.cpp L361
		breakset: "(){}:'",
		
		// engine/cmd.cpp L51/52
		maxAliasNameLength: 32,
		maxCmdLengthSometimes: 1024, // Used in BindToggle and alias only. Inexplicably also used in developer messages for voting (game/server/player.cpp L9834)
		aliases: {}, // engine/cmd.cpp *cmd_alias
		...
		parseArguments: function(text = '', printerrs = false) {
			...
			for (let cmd of cmds) {
				...
				if (cmd.length >= this.maxCmdLength - 1) {
					if (printerrs) this.parent.con.warn('Cbuf: Tokenizer encountered command which overflows the tokenizer buffer... Skipping!\n');
				...
				*/
				...
				if (args.length >= this.maxArgCount) {
					if (printerrs) this.parent.con.warn('Cbuf: Tokenizer encountered command which overflows the argument buffer... Clamping!\n');
		...
		executeAll: function() {
			...
			if (this.slowdown) {
				if (this.buffer.length > 0) this.awaitReadyExecute(this.buffer.shift());
				this.executing = false;
			} else {
				if (this.buffer.length > 0) this.awaitReadyExecute(this.buffer.shift(), true);
				this.awaitExecuteComplete();
			}
		},

		awaitReadyExecute: function(cmd, flush = false) {
			if (source.cmd.commandready) {
				source.cmd.commandready = false;
				source.cmd.execute(cmd);
				if (source.cmd.buffer.length > 0 && flush) source.cmd.awaitReadyExecute(source.cmd.buffer.shift(), true);
			} else {
				setTimeout(source.cmd.awaitReadyExecute, 1, cmd, flush);
			}
		},

		awaitExecuteComplete: function() {
			if (source.cmd.buffer.length == 0) {
				source.cmd.executing = false;
			} else {
				setTimeout(source.cmd.awaitExecuteComplete, 1);
			}
		},
		...
		*/
		execute: function(args) {
			...
			// WONTFIX: Execution markers. They seem to only half exist and I'm not sure what they even do. For posterity, try running "[$&*,`] something 1" ingame (paste it, since you can't enter ` in console). See also engine/cmd.cpp L843
			...
			if (!args[0])
				return this.commandready = true; // no tokens
			
			// Check for an alias
			{
				let alias = this.aliases[args[0].toLowerCase()];
				if (alias) {
					this.executeCommand(this.aliases[alias]);
					return this.commandready = true;
				}
			}
			...
			if (cmd) {
				if (cmd.isCommand) {
					...
					cmd.callback(args);
					// If a command is created with synchronous disabled, we will assume the command will, when it's done, set commandready
					if (cmd.synchronous) this.commandready = true;
					
					// FIXME: Post-hooks on asynchronous commands will be run synchronously
					if (this.parent.__.hooks[args[0]])
					...
					if (args.length == 1) {
						this.parent.con.log(this.parent.describeCvar(cmd));
						return this.commandready = true;
					}
					let value = args.slice(1).join(' ');
					this.parent.setCvar(cmd.name, value);
					return this.commandready = true;
				}
			} else {
				// Unknown command
		...
		compress: function(text = '') {
			...
			for (let i = 0; i < commands.length; i++) {
				...
				for (let j = 0; j < args.length; j++) {
					...
					//if ([...this.breakset].some(e => arg.includes(e)) && arg.length > 1) haveToQuote = true;
					if ([...this.breakset, ' ', ';'].some(e => ~arg.indexOf(e))) haveToQuote = true;

					// to be safe: just keep all quotes that already exist
					if (j - 1 > -1 && args.cmdStr.slice(args.argLengthS[j - 1] || 0)[0] == '"') haveToQuote = true;
					
					if (arg == '') haveToQuote = true;
					
					if (haveToQuote) outS += `"${arg}"`;
					else {
		...
		maybeQuote: function(str) {
			str = str.toString();
			let haveToQuote = false;
			if ([...this.breakset].some(e => ~str.indexOf(e)) && str.length > 1) haveToQuote = true;
	...
	con: {
		...
		enterCommand: function(command) {
			this.enterHistory.push(command);
			this.log(`] ${command}\n`);
			this.parent.cmd.executeCommand(command);
		},
		
		autocomplete: function(command, slice = 0, len = 5) {
			let completions = [...this.autocomplete_all(command)];
			return completions.slice(Math.max(0, slice)).slice(0, len);
		},
		
		autocomplete_all: function(command) {
			let cmds = this.parent.cmd.parseArguments(command);
			let args = cmds[cmds.length - 1];
			if (!args || args.length == 0) return [];
			if (args.length == 1 && !command.endsWith(' ')) {
				let startsWith = this.parent.cmds.filter(e => e.name.startsWith(args[0])).map(e => e.name).sort()
				let includes = this.parent.cmds.filter(e => ~e.name.indexOf(args[0])).map(e => e.name).filter(e => !startsWith.includes(e)).sort()
				return [...startsWith, ...includes, ...this.enterHistory];
			}
			let cmd = this.parent.getCmd(args[0]);
			if (!cmd) return [];
			return cmd.isCommand
				? [...cmd.autocompletefunc(args).map(e => args.cmdStr.slice(0, args.argLengthS[args.length - 2]) + e).sort()]
				:  args.length == 1 ? [command + this.parent.cmd.maybeQuote(cmd.value)] : [];
		}
	},
	...
	createCommand: function(name, helpStr = '\n', callback = (args) => {source.con.warn(`NotImpl: ${args[0]} (${args.cmdStr.trimStart()})\n`)}, autocompletefunc = function(cmd) {return []}, synchronous = true) {
		...
		if (~existing) {
			if (this.cmds[existing].isCommand) return this.cmds[existing] = obj;
	...
	setCvar: function(name, value) {
		...
		if (cvar) {
			if (cvar.isCommand) return false;
			if (cvar.min > Number.NEGATIVE_INFINITY) value = Math.max(cvar.min, value);
			if (cvar.max < Number.POSITIVE_INFINITY) value = Math.min(cvar.max, value);
			cvar.value = value;
	...
	unregister: function(name) {
		// Super lazy way of doing this. When I inevitably convert this.cmds to a pure Object rather than an Array, this will be "delete this.cmds[name];"
		this.cmds = this.cmds.filter(e => e.name != name);
	},

	test: {
		cmdParse: function(input = '', expected = []) {
			let output = this.parent.cmd.parseArguments(input);
			if (JSON.stringify(output) == JSON.stringify(expected)) {}//console.log('Test for <' + input + '> passed, got\n' + output.map(e => e.join(' ')).join('\n'));
			else {
				src.con.err('Command parsing error: From <' + input + '> got\n' + output.map(e => e.join(' ')).join('\n') + '\ninstead of expected\n', expected);
				console.error('From <' + input + '> got\n' + output.map(e => e.join(' ')).join('\n') + '\ninstead of expected\n', expected);
			}
		},
	},
...
3: mouse_acceleration = pow(raw_mouse_delta, m_customaccel_exponent - 1) * sensitivity`);
		...
		cmd('unbind', 'unbind <key> : remove commands from a key\n', function(args) {
			...
			let bind = source.binds.find(e => e.key == keyUp);
			if (bind) return bind.cmd = '';
			
			source.binds.push({key: keyUp, cmd: ''});
		});
		
		cmd('unbindall', 'Unbind all keys.\n', function(args) {
			let keys = source.binds.filter(e => !~['ESCAPE', '`'].indexOf(e.key));
			for (let key of keys) key.cmd = '';
		});
		...
		cmd('find', 'find <string> : find commands with the specified string in their name/help text\n', function(args) {
			...
			for (let cmd of source.cmds) {
				if (~cmd.name.toUpperCase().indexOf(search) || ~cmd.helpStr.toUpperCase().indexOf(search)) {
					if (cmd.isCommand) source.con.log(`"${cmd.name}" - ${cmd.helpStr}\n`);
		...
		cmd('alias', 'Alias a command.\n', function(args) {
			if (args.length == 1) {
				source.con.log(`alias: Current alias commands:\n`);
				let aliases = Object.keys(source.cmd.aliases);
				for (let alias of aliases) {
					source.con.log(`${alias} : ${source.cmd.aliases[alias]}\n`);
				}
				return;
			}
			
			let name = args[1].toLowerCase();
			if (name.length >= source.cmd.maxAliasNameLength)
				return source.con.err(`alias: Alias name is too long (${name})\n`);
			
			let cmd = args.slice(2).join(' ').slice(0, source.cmd.maxCmdLengthSometimes); // maxCmdLengthSometimes does nothing as it's limited to 510 before this point.
			
			if (source.cmd.aliases[name]) {
				if (source.cmd.aliases[name] == cmd) return;
				delete source.cmd.aliases[name];
			}
			
			let existing = source.getCmd(name);
			if (existing)
				return source.con.err(`alias: Cannot alias an existing ${existing.isCommand ? 'concommand' : 'convar'}\n`);
			
			source.cmd.aliases[name] = cmd;
		});
		
		cmd('plugin_load', 'plugin_load <filename> : loads a plugin\n', function(args) {
		
			// Extensible, just add <plugin_name>.js to plugins folder
			...
			if (source.plugins.find(e => e == name.toLowerCase())) {
				source.con.warn(`plugin_load: Plugin "${name}" is already loaded!\n`);
				return source.cmd.commandready = true;
			}

			let pluginscript = document.createElement('script');
			pluginscript.src = `plugins/${name}.js`;
			pluginscript.onerror = function() {
				source.con.err(`Unable to load plugin "${name}"\n`);
				source.cmd.commandready = true;
			}
			pluginscript.onload = function() {
				let hook = source.plugin_load_hooks.find(e => e.name == name.toLowerCase());
				if (hook) {
					hook.callback();
				}
				source.plugins.push(name.toLowerCase());
				source.con.log(`Loaded plugin "${name}"\n`);
				source.cmd.commandready = true;
			}
		...
		cmd('map', 'map <map> - go to map lol\n', function(args) {
			if (args.length != 2) {
				return source.__.wrongArgCount(args);
			
			}let match_maps = src.maps.filter(e => ~e.indexOf(args[1]));
			if (match_maps.length == 0) {
				return source.con.err('map dont exist\n');
			}
			let map = match_maps[0];
			source.prev_map = source.map;
			source.map = map;
		}, function(args) {
			return src.maps.filter(e => ~e.indexOf(args[1] || ''));
		});
		
		cmd('changelevel', 'changelevel <map> - go to map lol\n', function(args) {
			if (args.length != 2) {
				return source.__.wrongArgCount(args);
			}
			let match_maps = src.maps.filter(e => ~e.indexOf(args[1]));
		...
		cmd('disconnect', 'disconnect - wtf do you think this does\n', function(args) {
			if (source.map == '') {
				return source.con.err('not connected\n');
			}
			source.prev_map = source.map;
			source.map = '';
		});
		
		cmd('restart_level', 'restart_level - restarts level\n', function(args) {
			if (source.map == '') {
				return source.con.err('not connected\n');
			}
		});
		
		cmd('restart', 'restart - restarts level and disables cm\n', function(args) {
			if (source.map == '') {
				return source.con.err('not connected\n');
			}
			source.setCvar('sv_bonus_challenge', 0);
		});
	...
	__init: function() {
		...
		for (let obj of Object.keys(this)) {
			...
			if (!~['cmds', 'binds', 'plugins', 'plugin_load_hooks', 'onTickEvents'].indexOf(obj)) this[obj].parent = this;
		}
		this.__initVars();
		this.__initMaps();
		...
		requestAnimationFrame(() => {
			...
			if (source.cfg.get('autoexec') == null) source.cfg.add('autoexec', `
...
setTimeout(() => source.cmd.executeCommand('exec config.cfg', false, true), 600);

source.test.cmdParse('"this is" a test', [['this is', 'a', 'test']]);
source.test.cmdParse('echo hi', [['echo', 'hi']]);
source.test.cmdParse('echo\'hello', [['echo', "'", 'hello']]);
source.test.cmdParse('echo\'hello\'', [['echo', "'", 'hello', "'"]]);
source.test.cmdParse('echo hi; echo hello', [['echo', 'hi'], ['echo', 'hello']]);
source.test.cmdParse('echo (test)', [['echo', '(', 'test', ')']]);
source.test.cmdParse(Array.from({length: 65}, (v, i) => i + 1).join(' '), [Array.from({length: 64}, (v, i) => (i + 1).toString())]);
source.test.cmdParse("'".repeat(65), ["'".repeat('64').split('')]);
source.test.cmdParse('echo "hello \' there"', [['echo', 'hello \' there']]);
source.test.cmdParse('echo "hello // one" // two', [['echo', 'hello // one']]);
source.test.cmdParse('echo ("te)st"', [['echo', '(', 'te)st']]);
source.test.cmdParse('a(b)c{d}e:f\'g;h i; a"(b)"{c', [['a', '(', 'b', ')', 'c', '{', 'd', '}', 'e', ':', 'f', "'", 'g'], ['h', 'i'], ['a', '(b)', '{', 'c']]);
