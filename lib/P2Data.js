class P2Data {
	constructor() {
		...
		this.maps = [
			...
			P2Map(['sp_a1_intro2'                , 'Portal Carousel',      1, false, 62758, `Aperture Science Reintegration [As]sociate`]),
			P2Map(['sp_a1_intro3'                , 'Portal Gun',           1, false, 47458, `Asked that [first]. ... I'm just gonna [wait]`]),
			...
			P2Map(['sp_a2_dual_lasers'           , 'Dual Lasers',          2, false, 47738, `Congratulate you on beating [*] the odds`]),
			...
			P2Map(['sp_a2_bridge_intro'          , 'Bridge Intro',         3, false, 47752, `Guess 'sharks'? Because [that's] wrong`]),
			...
			P2Map(['sp_a3_jump_intro'            , 'Repulsion Intro',      6, false, 47787, `Lab boys gave me: [*] do not`]),
			P2Map(['sp_a3_bomb_flings'           , 'Bomb Flings',          6, false, 47468, `Had fluorescent [calcium] in it`]),
			...
			P2Map(['sp_a3_speed_ramp'            , 'Propulsion Intro',     7, false, 47791, `Help, we're gonna [change] the world`]),
			...
			P2Map(['sp_a4_tb_intro'              , 'Funnel Intro',         8, false, 47800, `The good [news] is...`]),
			P2Map(['sp_a4_tb_trust_drop'         , 'Ceiling Button',       8, false, 47802, `I know we're in a lot [of] trouble`]),
			P2Map(['sp_a4_tb_wall_button'        , 'Wall Button',          8, false, 47804, `Oh no... [*] it's happening`]),
			P2Map(['sp_a4_tb_polarity'           , 'Polarity',             8, false, 47806, ``]),
			P2Map(['sp_a4_tb_catch'              , 'Funnel Catch',         8, false, 47808, `Can get a little... [unbearable]`]),
		...
		this.mapWithProperty('filename', 'sp_a2_bts6').noCM = true;
		this.mapWithProperty('filename', 'sp_a3_00').noCM = true;
		this.mapWithProperty('filename', 'mp_coop_start').noCM = true;
		this.mapWithProperty('filename', 'mp_coop_lobby_3').noCM = true;
	...
	getCMChapterIdx() {
		...
		for (let i = 0; i < this.maps.length; i++) {
			if (this.maps[i].hasOwnProperty('noCM')) this.maps[i].cmChapterIdx = -1;
	...
	async getMtriggers() {
		...
		return queryAPI(this.maps.filter(e => !e.hasOwnProperty('noCM')).map(e => 
			'https://raw.githubusercontent.com/p2sr/portal2-mtriggers/master/' +
			(e.coop ? 'Coop/' : 'SP/') +
			e.chapter.toString().padStart(2, '0') + '_' + this.chapters[e.coop ? 'mp' : 'sp'][e.chapter - 1].toLowerCase().replaceAll(' ', '-') + '/' +
			e.cmChapterIdx.toString().padStart(2, '0') + '_' + e.splitname.toLowerCase().replaceAll(' ', '-') + '.cfg'
		)).then(e => {
			let count = e.length;
			for (let i = 0; i < count; i++) {
				let map = this.maps.filter(e => !e.hasOwnProperty('noCM'))[i];
