...
//		Rewrite to integrate with source.js
...
window.onload = async function() {
	...
	{
	let mt = `sar_speedrun_cc_start "Container Ride" map=sp_a1_intro1 action=split
...
sar_speedrun_cc_rule "Flags 1" flags
sar_speedrun_cc_rule "Flags 2" flags "ccafter=Flags 1" action=stop
sar_speedrun_cc_finish`.split('sar_speedrun_cc_start ').map(e => e.split('\n').filter(e => e)).filter(e => e.length);
	for (let i = 0; i < data.maps.length; i++) {
		let str = mt.filter(e => e[0].startsWith('"' + data.maps[i].splitname + '"'));
		if (str[0]) data.maps[i].triggers = str[0].filter(e => e.startsWith('sar_speedrun_cc_rule ')).map(e => e.substr(21));
	}
	}
